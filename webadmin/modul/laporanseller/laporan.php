<?php
	if( $_SESSION['email'] == "" AND $_SESSION['level'] == "" ){
		...
		switch ( $_GET['act'] ){
			default :
?>
			<form method="POST" action="modul/laporanseller/laporan-pdf.php" class="form-horizontal">
				<div class="row">
					<div class="col-sm-8">
					...
					<div class="col-sm-4">
						<p class="pull-right">
							<a href="modul/laporanseller/laporan-skrg.php" class="btn btn-success"><i class="glyphicon glyphicon-download"></i> Laporan Hari Ini</a>
							<button type="submit" name="cetak" class="btn btn-info"><i class="glyphicon glyphicon-print"></i></button>
							<button type="button" class="btn btn-default" data-toggle="tooltip" title="Refresh" onclick="window.location.href='?module=laporanseller';"><i class="glyphicon glyphicon-refresh"></i></button>
						</p>
					</div>
				</div>
				<div class="row">

		...
		<div class="container">
			...
			<?php
				...
				if ( empty($_GET['halproduk']) ){
					$posisi              = 0;
					$_GET['halproduk'] = 1;
				} else if ( $_GET['halproduk'] > 0 ){
					$posisi              = ( $_GET['halproduk'] - 1 ) * $batas;
				}
				$query5                  = $db->prepare("SELECT kategori_produk.id_kategori,kategori_produk.kategori_seo,produk.id_produk,produk.nama_produk,produk.produk_seo,produk.diskon,produk.stok,produk.harga,produk.gambar FROM produk LEFT JOIN subkategori_produk ON produk.id_subkategori=subkategori_produk.id_subkategori LEFT JOIN kategori_produk ON subkategori_produk.id_kategori=kategori_produk.id_kategori WHERE produk.aktif=? ORDER BY produk.id_produk DESC limit $posisi, $batas");
			...
			?>
				...
				<center>
					...
					<?php
						$query27      = $db->query("SELECT id_produk FROM produk");
						$row27        = $query27->num_rows;
						$jumlah_hal27 = ceil ($row27/$batas);
					?>
						<li class="<?php if ( $_GET['halproduk'] <= 1 ) echo "disabled"; ?>"><a href="halproduk-<?php echo $_GET['halproduk'] - 1; ?>.html"><i class="fa fa-angle-left">«</i></a></li>
						<?php
							for ( $i = 1; $i <= $jumlah_hal27; $i++ ){
						?>
							<li class="<?php if ( $_GET['halproduk'] == $i ) echo "active"; ?>"><a href="halproduk-<?php echo $i; ?>.html"><?php echo $i; ?></a></li>
						<?php
							}
						?>
						<li class="<?php if ( $_GET['halproduk'] >= $jumlah_hal27 ) echo "disabled"; ?>"><a href="halproduk-<?php echo $_GET['halproduk'] + 1; ?>.html"><i class="fa fa-angle-right">»</i></a></li>
					</ul>
				</center>
				</div>
			</div>
